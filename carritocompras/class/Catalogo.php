...
class Catalogo
{
	...
	public function get_libros_cat($idcat)
	{
		...
		$sql="select * from instrumentos where id_categoria=$idcat";
		$result=$conn->query($sql);
	...
	public function get_detalles_libro($isbn)
	{
		...
		$sql="select * from instrumentos where id_instrumento=$isbn";
		$result=$conn->query($sql);
	...
	public static function calcular_total($cart)
	{
		...
		if (is_array($cart))
		{
			...
			{
				$sql="select precio from instrumentos where id_instrumento=$isbn";
				$result=$conn->query($sql);
